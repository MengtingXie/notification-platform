...
func (s *FailoverManagerSuite) TestRecover() {
	...
	testCases := []struct {
		name   string
		before func(t *testing.T, svc config.ServiceInstance)
...
func (s *FailoverManagerSuite) TestTryTakeOver() {
	...
	testCases := []struct {
		...
		standBySvc config.ServiceInstance
		targetSvc  config.ServiceInstance
		...
		},
		{
			...
			before: func(t *testing.T, svc config.ServiceInstance) {
				...
				success, err := manager.TryTakeOver(t.Context(), config.ServiceInstance{
...
func (s *FailoverManagerSuite) TestWatchFailover() {
	...
	// 创建几个事件
	expectedEvents := []config.FailoverEvent{
		{
	...
	// 接收事件的协程
	var receivedEvents []config.FailoverEvent
...
func (s *FailoverManagerSuite) TestConcurrentTakeOver() {
	...
	type TakeOverEvent struct {
		config.FailoverEvent
		Holder config.ServiceInstance
	}
		...
		go func(id int) {
			...
			for {
				...
				case evt := <-failoverEvents:
					backupSvc := config.ServiceInstance{
						Group:   "",
						Name:    "backup",
...
type MockServer struct {
	...
	failMgr config.FailoverManager
...
func (s *MockServer) Failover(ctx context.Context) error {
	return s.failMgr.Failover(ctx, config.ServiceInstance{
...
func (s *MockServer) Recover(ctx context.Context) error {
	return s.failMgr.Recover(ctx, config.ServiceInstance{
		Group:   s.RegistryInfo.Group,
		Name:    s.RegistryInfo.Name,
		Address: s.RegistryInfo.Address,
	})
...
func (s *MockServer) watchFailoverEvents(ctx context.Context) {
	...
	for {
		...
		case evt, ok := <-eventCh:
			...
			succeed, err1 := s.failMgr.TryTakeOver(ctx, config.ServiceInstance{
				Group:   s.RegistryInfo.Group,
				Name:    s.RegistryInfo.Name,
				Address: s.RegistryInfo.Address,
			}, evt.ServiceInstance)
...
// 监听被接管 core 的恢复
func (s *MockServer) watchRecoveryEvents(ctx context.Context, target config.ServiceInstance) {
