...
var _ config.FailoverManager = (*Manager)(nil)
...
// Failover 标记服务需要故障转移
func (m *Manager) Failover(ctx context.Context, svc config.ServiceInstance) error {
...
// buildKey 构建故障键
func (m *Manager) buildKey(svc config.ServiceInstance) string {
	return fmt.Sprintf("%s/%s/%s/%s", failoverPrefix, svc.Name, svc.Group, svc.Address)
}

// Recover 标记服务已恢复可用
func (m *Manager) Recover(ctx context.Context, svc config.ServiceInstance) error {
	if err := svc.Validate(); err != nil {
		return err
	}
...
func (m *Manager) TryTakeOver(ctx context.Context, standbyService, targetService config.ServiceInstance) (bool, error) {
...
func (m *Manager) buildValue(svc config.ServiceInstance) string {
	return fmt.Sprintf("%s:%s:%s", svc.Group, svc.Name, svc.Address)
}

// WatchFailover 监听故障转移事件
func (m *Manager) WatchFailover(ctx context.Context) (<-chan config.FailoverEvent, error) {
	// 创建带取消的上下文
	...
	eventCh := make(chan config.FailoverEvent)
			...
			case resp := <-watchCh:
				...
				for _, event := range resp.Events {
					...
					if event.Type == clientv3.EventTypePut && string(event.Kv.Value) == "" {
						...
						select {
						case eventCh <- config.FailoverEvent{
...
func (m *Manager) parseKey(key string) (*config.ServiceInstance, error) {
...
func (m *Manager) WatchRecovery(ctx context.Context, svc config.ServiceInstance) (<-chan struct{}, error) {
	if err := svc.Validate(); err != nil {
		return nil, err
	}
