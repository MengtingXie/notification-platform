...
// GetByIDsV1 第一种写法，不是精细控制的
func (b *businessConfigRepository) GetByIDsV1(ctx context.Context, ids []int64) (map[int64]domain.BusinessConfig, error) {
	result, err := b.localCache.GetConfigs(ctx, ids)
	if err != nil {
		b.logger.Error("从本地缓存批量获取失败", elog.FieldErr(err))
	}
	...
	// 不做精细控制
	result, err = b.redisCache.GetConfigs(ctx, ids)
	if err != nil {
		b.logger.Error("从 Redis 缓存批量获取失败", elog.FieldErr(err))
	}
	// 全找到了
	if len(result) == len(ids) {
		// 这里不要忘了回写本地缓存
		err = b.localCache.SetConfigs(ctx, mapx.Values(result))
		if err != nil {
			b.logger.Error("回写本地缓存失败", elog.FieldErr(err))
		}
		return result, nil
	}

	configs, err := b.dao.GetByIDs(ctx, ids)
	if err != nil {
		return nil, err
	}
	...
	if len(result) > 0 {
		vals := mapx.Values(result)
		// 回写
		err = b.localCache.SetConfigs(ctx, vals)
		if err != nil {
			b.logger.Error("批量回写本地缓存", elog.FieldErr(err))
		}

		err = b.redisCache.SetConfigs(ctx, vals)
		if err != nil {
			b.logger.Error("批量回写 Redis 缓存", elog.FieldErr(err))
		}
	}
...
func (b *businessConfigRepository) Delete(ctx context.Context, id int64) error {
	...
	err = b.redisCache.Del(ctx, id)
	if err != nil {
		b.logger.Error("删除redis缓存失败", elog.FieldErr(err), elog.Int64("bizId", id))
	}
	err = b.localCache.Del(ctx, id)
	if err != nil {
		b.logger.Error("删除本地缓存失败", elog.FieldErr(err), elog.Int64("bizId", id))
	}
	return nil
}
...
func (b *businessConfigRepository) SaveConfig(ctx context.Context, config domain.BusinessConfig) error {
	...
	if err != nil {
		b.logger.Error("更新redis缓存失败", elog.FieldErr(err), elog.Int64("bizId", config.ID))
	}

	err = b.localCache.Set(ctx, b.toDomain(cfg))
	if err != nil {
		b.logger.Error("更新本地缓存缓存失败", elog.FieldErr(err), elog.Int64("bizId", config.ID))
	}
	return nil
}
