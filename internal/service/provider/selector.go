...
func (r *RoundRobinSelector) Next(ctx context.Context, notification domain.Notification) (Provider, error) {
	if len(r.providers) == 0 {
		return nil, ErrNoAvailableProvider
	}
	r.idx++
	return r.providers[r.idx%len(r.providers)], nil
}

type SelectorBuilder struct {
	logger *elog.Component
	// 阿里云 SMS provider，腾讯云 SMS provider
	providers []Provider
}
...
func (s *SelectorBuilder) Build() Selector {
