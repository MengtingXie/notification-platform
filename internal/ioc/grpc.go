...
func InitGrpc(noserver *grpcapi.NotificationServer, configSvc *grpcapi.ConfigServer, etcdClient *eetcd.Component) *egrpc.Component {
	// 注册全局的注册中心
	type Config struct {
		Key string `yaml:"key"`
	}
	...
	configv1.RegisterBusinessConfigServiceServer(server.Server, configSvc)
